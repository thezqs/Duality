...
## una funcion auxiliar para actualizar el slider
func update_slider(time: int = -1):
	if time == -1: %TimeVSilder.value = Editor.current_time_steps
	
	%TimeVSilder.value = time

## una funcion auxiliar para avanzar en steps
func offset_seek(advance: int):
	Editor.current_time_steps += advance
	
	Manager.current_time = Editor.steps_to_seconds(Editor.current_time_steps)
	update_slider()

func create_note(pos: Vector2):
	var pos_x = (pos.x - 288) * 2
...
#region inicio y prosesamiento en fotogramas
...
#region edicion de notas e input
